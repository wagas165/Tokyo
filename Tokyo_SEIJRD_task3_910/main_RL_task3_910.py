...
def evaluate_model(model_path, algo, exp_id, days=20,
                   a_cost_coef=1.0, beta2_k=BETA2_K, commute_coef=0.0, is_states=False):
    if algo == "PPO":
    ...
    while not done:
        ...
        ep["hourly_reward"].append(float(rew))
        if step % 24 == 0:
            ep["isolation_series"].append(np.clip(action,0,1).tolist())
        step += 1

    results = {"method": f"{algo}_exp{exp_id}", "episodes":[ep]}
    with open(os.path.join(out_dir, "evaluation.json"), "w") as f:
        json.dump(results, f, indent=2)

    # 单实验时序图（含 cumulative reward）
    _plot_last_episode_time_series(ep, os.path.join(out_dir,"evaluation_timeseries.png"))

    # 与所有 Task-3 baseline 的对比图（四联图）
    _draw_strategy_comparison_single_experiment(ep, out_path=os.path.join(out_dir,"strategy_comparison.png"))

    print(f"[Exp {exp_id}] evaluate done => {out_dir}")
    return results
...
def parallel_evaluate(configs):
    ...
    tasks=[]
    for cfg in configs:
        algo = cfg["algo"]; eid=cfg["exp_id"]
        mpath=f"./models/{algo}_exp{eid}_final.zip"
        if os.path.exists(mpath):
            tasks.append( (mpath, algo, eid, cfg.get("days",20),
                           cfg.get("a_cost_coef",1.0), cfg.get("beta2_k",BETA2_K),
                           cfg.get("commute_coef",0.0), cfg.get("is_states",False)) )
        else:
            print(f"Model missing: {mpath}")
    with multiprocessing.Pool(processes=min(len(tasks), os.cpu_count())) as pool:
